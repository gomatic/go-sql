...
3. **Verbatim leaves.** Each AST leaf (expression, literal, qualified name) is rendered by slicing its `[Start,End)` bytes from the source via the token stream — so the formatter *cannot* misrender or drop an expression it doesn't structurally understand. Unknown/complex subtrees render as their verbatim source span, never a placeholder.
...
- R8 — Honest 100% coverage: tests assert the *contract* (specific round-trips, comment placement, the verification gate firing), never pin placeholder/destructive output.
