...
func TestAlterTableIdentity(t *testing.T) {
	...
	tests := []alterIdentityCase{
		{
			name:     "add_column",
			sql:      "ALTER TABLE my_schema.my_table ADD COLUMN new_col text",
			expected: "alter.table.add_column:my_schema.my_table:new_col",
		},
		{
			name:     "add_constraint",
			sql:      "ALTER TABLE my_table ADD CONSTRAINT my_pk PRIMARY KEY (id)",
			expected: "alter.table.add_constraint:my_table:my_pk",
		},
		{
			name:     "drop_column",
			sql:      "ALTER TABLE my_table DROP COLUMN old_col",
			expected: "alter.table.drop_column:my_table:old_col",
		},
		{
			name:     "drop_constraint",
			sql:      "ALTER TABLE my_table DROP CONSTRAINT my_fk",
			expected: "alter.table.drop_constraint:my_table:my_fk",
		},
		{
			name:     "drop_not_null",
			sql:      "ALTER TABLE my_table ALTER COLUMN col DROP NOT NULL",
			expected: "alter.table.drop_not_null:my_table:col",
		},
		{
			name:     "set_not_null",
			sql:      "ALTER TABLE my_table ALTER COLUMN col SET NOT NULL",
			expected: "alter.table.set_not_null:my_table:col",
		},
	}
...
func TestAlterTableIdentity_RawCases(t *testing.T) {
	...
	tests := []alterIdentityRawCase{
		...
		{
			name:     "no_cmds_is_unknown",
			stmt:     wrap(map[string]any{"relation": map[string]any{"relname": "t"}}),
		...
		},
		{
			...
			stmt: wrap(
				map[string]any{
					"relation": map[string]any{"relname": "t"},
					"cmds":     []any{map[string]any{"node": map[string]any{"alter_table_cmd": map[string]any{}}}},
				},
			),
			expected: "alter.table.unknown:t",
		},
...
func TestExtractAlterTarget(t *testing.T) {
	...
	tests := []alterTargetCase{
		...
		{
			name: "add_column_from_def",
			cmd: statementData{
				"def": map[string]any{"node": map[string]any{"column_def": map[string]any{"colname": "c"}}},
			},
			subtype:  alterAddColumn,
			expected: "c",
		},
		{
			name: "add_constraint_from_def",
			cmd: statementData{
				"def": map[string]any{"node": map[string]any{"constraint": map[string]any{"conname": "pk"}}},
			},
			subtype:  alterAddConstraint,
			expected: "pk",
		},
	...
	tests := []alterCompareCase{
		{
			...
			sourceSQL:   "ALTER TABLE t ADD COLUMN c text",
			targetSQL:   "ALTER TABLE t ADD COLUMN c int",
			expectDiffs: true,
		},
		{
			name:        "identical_add_column",
			sourceSQL:   "ALTER TABLE t ADD COLUMN c text",
			targetSQL:   "ALTER TABLE t ADD COLUMN c text",
			expectDiffs: false,
		},
	}
